...
class YoloFastestV2:
    def __init__(self, onnx_path, names, backend='onnx', objThreshold=0.3, confThreshold=0.3, nmsThreshold=0.4):
        ...
        self.anchors = np.array(
            [27.64,79.85, 39.01,83.49, 53.02,83.39, 64.78,84.64, 67.39,120.88, 81.16,85.94],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str, default=r'D:\work\light_objectdetect\img', 
                        help='The path of test image')
    opt = parser.parse_args()
    for root, dirs, files in os.walk(opt.imgpath):
        for name in files:
            name1=name.split('.')[0]+".jpg"
            imgpath=os.path.join(opt.imgpath,name1)
            srcimg = cv2.imread(imgpath)
            model = YoloFastestV2(onnx_path='static/JBoxafter11.onnx', names='static/my.names',
                                backend='onnx',
                                objThreshold=0.3,
                                confThreshold=0.3,
                                nmsThreshold=0.4)
            ...
            cv2.imwrite("D:/work/light_objectdetect/results/"+name1, frame)
